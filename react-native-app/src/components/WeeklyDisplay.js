...
const WeeklyDisplay = ({ timeDifference }) => {
  ...
  //DO NOT DELETE THE FOLLOWING COMMENTED CODE
  const { loading: averageLoading, error: averageError, data: averageData } = useQuery(GET_WEEKLY_AVERAGE, {
    variables: { timezone: timeDifference },
  });
  const { loading: compositionLoading, error: compositionError, data: compositionData } = useQuery(GET_WEEKLY_COMPOSITION, {
    variables: { timezone: timeDifference },
  });
