...
const riceImage = require('../carbon-footprint/tests/rice_image');
...
const CATEGORY_WITH_DATA = [{ avgCarbonFootprint: 10, periodNumber: 0 },
...
const CATEGORY_EMPTY = [{ avgCarbonFootprint: 0, periodNumber: 0 },
...
describe('testing resolvers', () => {
  ...
  test('getCarbonFootprintFromImage: rice image', async () => {
    jest.setTimeout(30000);
    const file = { base64: riceImage };
    const actual = await resolvers.Mutation.postPicture(null, { file }, { dataSources, user });
    const expected = { name: 'rice', carbonFootprintPerKg: 1.14 };
    expect(actual).toEqual(expected);
  });
  ...
  test('Test postRecipe with a url', async () => {
    ...
    const actual = await resolvers.Mutation.postRecipe(null, { name }, { dataSources, user });
    const expected = {
      carbonFootprintPerKg: 1.63,
