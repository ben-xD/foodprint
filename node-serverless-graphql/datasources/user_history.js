class userHistAPI {
  ...
  async searchData(user_id) {
    const itemList = this.store.userHist.findOne({ user_id });
    return itemList
      ? {
        item: itemList.item,
  ...
  async avg_co2_for_user(carbonAPI, user) {
    ...
    const average = await this.average_data(carbonAPI, user_data);
    return average;
  }
  ...
  async weekly_average_cf(carbonAPI, user, timezone) {
    ...
    const average = +(sum / (this.NUMBER_OF_WEEKS_RETURNED - 1)).toFixed(2); // -1 because the current week is not taken into account
  ...
  async monthly_average_cf(carbonAPI, user, timezone) {
    ...
    const average = +(sum / (this.NUMBER_OF_MONTHS_RETURNED - 1)).toFixed(2); // -1 because the current week is not taken into account
    return average;
  }
  ...
  async convert_date(date, timezone) {
    const adjusted_date = new Date(date.getTime() + (timezone * 60000 * 60)).toISOString();
  ...
  async average_data(carbonAPI, data) {
    ...
    const n_items = repetitions.reduce((a, b) => a + b);
      ...
      if (carbonResult.carbonpkilo !== undefined) {
        ...
        for (let cat = 1; cat < 5; cat++) {
          if (categories.includes(cat)) {
            const cf = +carbonResult.carbonpkilo.toFixed(2);
