...
const userHistAPI = new UserHistAPI(store);
...
describe('User history database (mocked dataSources)', () => {
  test('Average co2 of all products for user x is 2.28 (rounded up)', async () => {
    ...
    const user = 'x';
    const actual = await userHistAPI.avg_co2_for_user(carbonAPI, user);
    const expected = 2.28;
  ...
  test('Calculate the weekly average co2', async () => {
    ...
    const timezone = 0;
    const user = 'x';
    const actual = await userHistAPI.weekly_average_cf(carbonAPI, user, timezone);
    const expected = 1.36;
    expect(actual).toEqual(expected);
  });
  ...
  test('Calculate the monthy average co2', async () => {
    ...
    const timezone = 0;
    const user = 'x';
    const actual = await userHistAPI.monthly_average_cf(carbonAPI, user, timezone);
    const expected = 3.2;
    expect(actual).toEqual(expected);
  });
  ...
  test('Get weekly_cf_composition', async () => {
    ...
    jest.spyOn(userHistAPI, 'get_week_i_data').mockImplementation(() => [{ item: 'rice' }, { item: 'orange' }]);
  ...
  test('Get monthly_cf_composition', async () => {
    jest.setTimeout(10000);
    jest.spyOn(userHistAPI, 'get_month_i_data').mockImplementation(() => [{ item: 'mock' }]);
  ...
  test('Delete test user data from database', async () => {
    // spy on Store, instead of calling the real thing...
    // how to mock store, we don't have access to it here
    const spy = jest.spyOn(store.userHist, 'deleteMany').mockImplementation(() => { });
    // TODO what is the response back from mongoose? (errors, etc. Make more tests from this)
    const fakeUserId = 'JohnnyBravo';
    userHistAPI.deleteUserData(fakeUserId);
