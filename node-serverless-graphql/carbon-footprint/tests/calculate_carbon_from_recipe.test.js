...
describe('getCarbonFootprintFromRecipe (mocked dataSources)', () => {
  test('Roasted chickpea wraps has co2/kg 10', async () => {
    ...
    const actual = await getCarbonFootprintFromRecipe(dataSources, url);
    const expected = {
      carbonFootprintPerKg: 10,
  ...
  test('A website with no recipe should return nulls', async () => {
    ...
    const actual = await getCarbonFootprintFromRecipe(dataSources, url);
    const expected = {
      carbonFootprintPerKg: null, item: null, ingredients: null, sourceUrl: null, imageUrl: null,
    };
    expect(actual).toEqual(expected);
  });
  ...
  test('Muhsroom risotto has co2/kg 10', async () => {
    ...
    const actual = await getCarbonFootprintFromRecipe(dataSources, name);
    const expected = {
      carbonFootprintPerKg: 10,
